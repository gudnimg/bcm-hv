...
// Data structure for data specific to individual module.
typedef struct {
	char partno[19];   // Part number.
	char revision[19]; // Firmware and Hardware revision.
	char location[3];  // Factory location. "AP"
	char serialno[17]; // Serial number.
	char date[5];      // Manufacturing date. "YYWW"
} sModule_t;

typedef struct _BCM4414 {
	uint8_t              page;
	uint8_t              operation;
	sModule_t            module_specific;
