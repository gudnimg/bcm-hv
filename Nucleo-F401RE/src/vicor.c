...
void read_status_cml(I2C_HandleTypeDef *hi2c) {
    uint8_t buffer[1] = {0};
    pmbus_read(hi2c, VICOR_CMD_STATUS_CML, buffer, 1);
...
void read_status_specific(I2C_HandleTypeDef *hi2c) {
    uint8_t buffer[1] = {0};
    pmbus_read(hi2c, VICOR_CMD_STATUS_MFR_SPECIFIC, buffer, 1);
...
void read_status_iout(I2C_HandleTypeDef *hi2c) {
    uint8_t buffer[1] = {0};
    pmbus_read(hi2c, VICOR_CMD_STATUS_IOUT, buffer, 1);
...
void read_status_input(I2C_HandleTypeDef *hi2c) {
    uint8_t buffer[1] = {0};
    pmbus_read(hi2c, VICOR_CMD_STATUS_INPUT, buffer, 1);
...
void read_temperature(I2C_HandleTypeDef *hi2c) {
    uint8_t buffer[2] = {0};
    pmbus_read(hi2c, VICOR_CMD_READ_TEMPERATURE_1, buffer, 2);
...
void read_iout(I2C_HandleTypeDef *hi2c) {
    uint8_t buffer[2] = {0};
    pmbus_read(hi2c, VICOR_CMD_READ_IOUT, buffer, 2);
...
void read_iin(I2C_HandleTypeDef *hi2c) {
    uint8_t buffer[2] = {0};
    pmbus_read(hi2c, VICOR_CMD_READ_IIN, buffer, 2);
...
void read_pout(I2C_HandleTypeDef *hi2c) {
    uint8_t buffer[2] = {0};
    pmbus_read(hi2c, VICOR_CMD_READ_POUT, buffer, 2);
...
void read_vin(I2C_HandleTypeDef *hi2c) {
    uint8_t buffer[2] = {0};
    pmbus_read(hi2c, VICOR_CMD_READ_VIN, buffer, 2);
...
void clear_faults(I2C_HandleTypeDef *hi2c) {
    ...
    const uint8_t address = DEVICE_ADDRESS << 1;
    HAL_I2C_Master_Transmit(hi2c, address, buffer, 1, I2C_TIMEOUT);
}